...
export async function transcribe(episodeId: string, audioUrl?: string | null, sourceType?: string): Promise<void> {
  ...
  if (!transcript) {
    await query("UPDATE episodes SET status = 'error', error_message = 'No transcript available' WHERE id = $1", [
      episodeId,
    ])
  ...
  // Save transcript
  await query('UPDATE episodes SET transcript = $1, transcript_source = $2 WHERE id = $3', [
    transcript,
    transcriptSource,
    episodeId,
  ])
