...
import { query, close as closeDb } from './lib/db.js';
import { enqueue, stats, close as closeQueue } from './lib/queue.js';
  ...
  switch (command) {
    ...
    case 'status': {
      ...
      for (const row of episodeCount.rows) {
        console.log(`  ${(row as { status: string; count: number }).status}: ${(row as { status: string; count: number }).count}`);
      }
      break;
    }
